#!/usr/bin/env python3
"""
CNN-Based Blood Pressure Prediction Training Script
...
- Lightweight architecture optimized for 32x32 images
- Better gradient flow for BP prediction task
"""
...
class CNNBPPredictor(nn.Module):
    ...
    def __init__(self, cnn_config: dict, bilstm_config: dict, attention_config: dict,
        ...
        if attention_config.get('use_physiological_features', True):
    ...
    def forward(self, x_seq, return_attention=False):
        ...
        # Add physiological features (dummy for now - can be enhanced)
        if hasattr(self, 'use_physiological_features') and self.use_physiological_features:
            phys_features = torch.randn(batch_size, seq_len, 16, device=features.device) * 0.1
...
def main():
    parser = argparse.ArgumentParser(description='CNN-Based BP Predictor Training')
    ...
    print("="*60)
    print("CNN-BASED BP PREDICTOR TRAINING")
    print("="*60)
    print("Key features:")
    ...
    print("- Same loss function as VAE version")
    print("="*60)
